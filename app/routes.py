...
def creation_depart_conditions(selected_vol):
    ...
    for x in liste_numero_securite_sociale:
        ...
        cur.execute("SELECT ts_arrivee,ville_destination,pays_destination FROM departs JOIN vols ON vols.num_vol = departs.num_vol JOIN airline.liaisons ON vols.liaison = liaisons.id_liaison JOIN (SELECT id_aeroports AS id_aeroport_destination, code AS code_destination, nom AS nom_destination, ville AS ville_destination, pays AS pays_destination FROM airline.aeroports) AS aeroports_destination ON liaisons.aeroport_destination = aeroports_destination.id_aeroport_destination WHERE vols.ts_arrivee = (SELECT MAX(ts_arrivee) AS ts_depart_next_flight FROM ((SELECT pilote_1 AS naviguant, ts_arrivee FROM airline.departs JOIN airline.vols ON departs.num_vol = vols.num_vol) UNION (SELECT pilote_2 AS naviguant, ts_arrivee FROM airline.departs JOIN airline.vols ON departs.num_vol = vols.num_vol) UNION (SELECT equipage_1 AS naviguant, ts_arrivee FROM airline.departs JOIN airline.vols ON departs.num_vol = vols.num_vol) UNION (SELECT equipage_2 AS naviguant, ts_arrivee FROM airline.departs JOIN airline.vols ON departs.num_vol = vols.num_vol)) AS total_naviguant WHERE total_naviguant.naviguant = %s AND ts_arrivee < %s GROUP BY naviguant)",(x,ts_depart_format))
        data = cur.fetchall()
        print(data)
...
def pilotage_personnel():
    ...
    cur.execute("SELECT nom, prenom,fonction,naviguants_vols.naviguant, naviguants_vols.num_vol FROM ((SELECT pilote_1 AS naviguant, num_vol FROM airline.departs) UNION DISTINCT (SELECT pilote_2 AS naviguant, num_vol FROM airline.departs) UNION DISTINCT (SELECT equipage_1 AS naviguant, num_vol FROM airline.departs) UNION DISTINCT (SELECT equipage_2 AS naviguant, num_vol FROM airline.departs)) AS naviguants_vols JOIN airline.vols ON vols.num_vol = naviguants_vols.num_vol JOIN airline.employes ON employes.numero_securite_sociale = naviguants_vols.naviguant JOIN airline.naviguants ON naviguants.numero_securite_sociale = naviguants_vols.naviguant WHERE ts_depart < NOW() AND ts_arrivee > NOW();")
